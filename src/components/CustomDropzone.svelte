...
<div class="max-sm:w-full w-2/5">
  ...
  >
    ...
    <div class="flex w-full gap-3.5">
      ...
      {#each videos as video, index}
        <div class={`relative shadow ${shapeStyle(shape)}`}>
          ...
          {#if !readonly}
            <button
              class={`${shapeClose(shape)} absolute bg-red-200 cursor-pointer`}
              on:click|preventDefault={() => handeModel(index, "video")}
            >
      ...
      {#each pdfs as pdf, index}
        ...
        >
          ...
          {#if !readonly}
            <button
              class={`${shapeClose(shape)} absolute bg-red-200 cursor-pointer`}
              on:click|preventDefault={() => handeModel(index, "pdf")}
            >
              <img src={CancelIcon} alt="close button img" />
            </button>
          {/if}
      ...
      {#if showWider}
        <button
          class={`w-2/4 flex justify-center items-center border-dashed border-2 cursor-pointer border-brand-Primary border-opacity-50 bg-BG-Secondary ${shapeStyle(shape)}`}
          on:click|preventDefault={handleUpload}
        >
          <img
            src={AddMain}
